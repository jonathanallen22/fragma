...
function updateLiveText() {
    ...
    // NOTA: Qui ho rimesso la logica base del tuo codice vecchio
    // se usavi il modulo %, assicurati che funzioni con i dati attuali
    const originalData = CARDS_DATA[currentCardIndex % CARDS_DATA.length]; 
...
function goBack() {
    if (sceneMgr.currentScene === 'carousel') { 
        // MODIFICA INFINITE SCROLL (LOGICA MATEMATICA VECCHIA)
...
const handleArduinoData = (data) => {
    ...
    if (data.e && data.e !== "NONE") {
        if (data.e === "CW") { 
            // MODIFICA INFINITE SCROLL ARDUINO (LOGICA VECCHIA)
            if (sceneMgr.currentScene === 'carousel') { 
...
document.addEventListener('keydown', (e) => {
    ...
    if (sceneMgr.currentScene === 'carousel' && e.key === 'ArrowRight') { 
        // MODIFICA INFINITE SCROLL TASTIERA (LOGICA VECCHIA)
        currentCardIndex = (currentCardIndex + 1) % CARDS_DATA.length; 
        updateCarousel(); 
    }
...
function handleConfirmation() {
    if (sceneMgr.currentScene === 'intro') sceneMgr.goTo('instructions');
        ...
        // --- MODIFICA CRUCIALE PER SOFT RESET ---
        const overlay = document.getElementById('final-confirmation-overlay');
        if (overlay) {
            if (overlay.classList.contains('visible')) {
                // PRIMA ERA: window.location.reload();
                // ORA: Chiamiamo la funzione che resetta tutto senza staccare Arduino
                softResetExperience();
            } else {
...
// --- NUOVA FUNZIONE DI SOFT RESET (Mantiene connessione Seriale) ---
function softResetExperience() {
    console.log("🔄 SOFT RESET EXPERIENCE...");

    // 1. Ferma intervallo carosello finale
    if (impactCarouselInterval) {
        clearInterval(impactCarouselInterval);
        impactCarouselInterval = null;
    }
    ...
    // 3. Nascondi Scena Impact V2
    const impactScene = document.getElementById('scene-impact-v2');
    if(impactScene) {
        impactScene.classList.remove('active');
        impactScene.classList.add('hidden');
    }

    // 4. Reset Variabili interne
    currentGeneratedBody = "";
    isEditInteractionStarted = false;
    currentCardIndex = 0; // Torna alla prima card

    // 5. Torna alla Intro (o alla scena che preferisci)
    sceneMgr.goTo('intro'); 

    // 6. Reset Carosello Notizie
    updateCarousel(); 
}
...
function setupFinalInteraction() {
    ...
    // --- MODIFICA CRUCIALE PER IL CLICK MOUSE ---
    // Invece di reload, usa soft reset
    overlay.onclick = () => { softResetExperience(); };
    ...
    setTimeout(() => { scene.addEventListener('click', showOverlay); }, 1000); 
}
